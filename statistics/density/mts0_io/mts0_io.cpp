...
void Mts0_io::read_mts(char *filename, vector<int> &atom_types_local, vector<int> &atom_ids_local, vector<vector<double> > &positions_local, vector<vector<double> > &velocities_local) {
	...
	for(int i=0;i<num_atoms_local;i++) {
		...
		int tmp1 = (int) ((tmp_atom_data[i]-atom_types_local[i])*1e11 + 0.5);
		int tmp2 = (int) round((tmp_atom_data[i]-atom_types_local[i])*1e11);
		if (tmp1 != tmp2) 
		{	cout << "Error in mts0_io.cpp while extracting atom_id from atom_data." << endl;
			cout << "num+0.5: " << tmp1 << ", int(round) : " << tmp2 << ", double: " << (tmp_atom_data[i]-atom_types_local[i])*1e11 << endl;
			exit(1);
		}
		atom_ids_local[i] = tmp1;
...
void Mts0_io::load_atoms(string mts0_directory, vector<int> node_id_list){
    positions.clear();
    velocities.clear();
    atom_types.clear();
    atom_ids.clear();
...
void Mts0_io::write_to_xyz_file(string output_file_name) {
    if(did_remove_at_least_one_atom) rearrange_vectors_by_moving_atoms_from_end_to_locations_where_atoms_have_been_removed();

    char *type[] = {(char*)"Not in use", (char*)"Si",(char*)"A ",(char*)"H ",(char*)"O ",(char*)"Na",(char*)"Cl",(char*)"X "};

    ofstream ofile(output_file_name.c_str());
    ofile << positions.size() << endl;
    ofile << "Comment" << endl;
    for (int atom_index = 0; atom_index < positions.size(); atom_index++) {
        ofile << type[atom_types[atom_index]] << " ";
        ofile << positions[atom_index][0] << " " << positions[atom_index][1] << " " << positions[atom_index][2] << " ";
        ofile << velocities[atom_index][0] << " " << velocities[atom_index][1] << " " << velocities[atom_index][2] << " ";
        ofile << atom_ids[atom_index];
        ofile << endl;
    }
}
